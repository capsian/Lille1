        ...
        for state in range(env.nS):
            ...
            for action in range(env.nA):
                ...
                for transition in transitions_info:
                    ...
                    next_state = transition[1]
                    reward = transition[2]
                    Q[action] += probability * (reward + GAMMA * v_opt_old[next_state])
            v_opt[state] = np.max(Q)
        delta_inf[i] = np.max(np.abs(v_opt_old - v_opt))
    ...
    for state in range(env.nS):
        for action in range(env.nA):
            ...
            for transition in transitions_info:
                ...
                next_state = transition[1]
                reward = transition[2]
                Q[state, action] += probabilities * (reward + GAMMA * v_opt[next_state])
        ...
        for state in range(env.nS):
            for action in range(env.nA):
                ...
                for transition in transitions_info:
                    ...
                    next_state = transition[1]
                    reward = transition[2]
                    q_opt[state][action] += probabilities * (reward + GAMMA * np.max(q_opt_old[next_state]))
        delta_inf[i] = np.max(np.abs(q_opt_old - q_opt))
