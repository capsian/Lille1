...
SCRIPT=$(pwd)/src/spatiotemporal_px.py
OUT_DIR=$(pwd)/out/spatiotemporal_px
...
if [[ -n "$THRESHOLD" ]]; then
    if [[ -n "$SKIP" ]]; then
        python3 $SCRIPT -o $org_img -p $past_img_arr -f $future_img_arr -out $FILENAME -c $calculate -t $THRESHOLD -s $SKIP &
    else
        python3 $SCRIPT -o $org_img -p $past_img_arr -f $future_img_arr -out $FILENAME -c $calculate -t $THRESHOLD &
    fi
else
    if [[ -n "$SKIP" ]]; then
        python3 $SCRIPT -o $org_img -p $past_img_arr -f $future_img_arr -out $FILENAME -c $calculate -s $SKIP &
    else
